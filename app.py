    ...
    if request.method=='POST':
        ...
        try:
            ...
            if email_count[0]==0:
                ...
                return redirect(url_for('aotp',paotp=encode(data=admindata))) #passing encrpty otp
...
def adminforgot():
    if request.method=='POST':
        ...
        if stored_email[0]==1:
            ...
            flash('No email registed pls check')
            return redirect(url_for('adminforgot'))
...
def delete_item(item_id):
    if session.get('admin'):
        try:
            ...
            stored_imgname=cursor.fetchone() #('Fg5Hg6.png',)
...
def updateitem(item_id):
    if session.get('admin'):
        try:
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select bin_to_uuid(item_id),item_name,price,quantity,category,image_name,added_by,description from items where item_id=uuid_to_bin(%s)',[item_id])
            stored_itemdata=cursor.fetchone() #(D35d,)
        except Exception as e:
            print(e)
            flash('Connection problem')
            return redirect(url_for('viewallitems'))
            ...
            if request.method=='POST':
                ...
                if filename== '': #not uploading img
                    ...
                    img_name=genotp()+'.'+filename.split('.')[-1] #uploading new img
                    drname=os.path.dirname(os.path.abspath(__file__)) #D:\pfs6\ecom
                    static_path=os.path.join(drname,'static')
    ...
    if request.method=='POST':
        ...
        try:
            ...
            if email_count[0]==0:
                ...
                flash('Otp has sent to given mail')
                return redirect(url_for('uotp',puotp=encode(data=userdata))) #passing encrpty otp
            elif email_count[0]==1:
                flash('Email already existed')
...
def uotp(puotp):
    if request.method=='POST':
        fotp=request.form['otp'] # user given otp
        try:
            dotp=decode(data=puotp) #decoding the admindata 
        except Exception as e:
            flash('something went wrong')
            return redirect(url_for('usercreate'))
        else:
        ...
        if request.method=='POST':
            ...
            try:
                ...
                if email_count[0]==1:
                    ...
                    stored_password=cursor.fetchone()
                    if stored_password[0].decode('utf-8')==password:
                        session['user']=uemail #{'user':'ambica@codegnan.com'}}
                        if not session.get(uemail):
                            session[uemail]={}   #{user:'ambica@codegnan.com','ambica@codegnan.com':{}}
    ...
    if session.get('user'):
        if request.method=='POST':
            ...
            try:
                ...
                print(f'Error ininserting review{e}')
